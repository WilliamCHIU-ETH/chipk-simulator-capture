...
function ocrLines(imagePath, lang = 'chi_tra', options = {}) {
  ensureTesseract();
  // Keep runtime scratch beside the caller-owned image so acquisition never writes
  // outside the caller's output directory.
  const base = path.join(path.dirname(imagePath), '.ocr_' + process.pid + '_' + Date.now());
  try {
    const args = [imagePath, base, '-l', lang];
